...
void gridify(int width, int height, int *hue_array, struct section *found_candidates_x, struct section *found_candidates_y)
{
	...
	*/
	...
	{ // iterating by row
		...
		{ // iterating through each element one by one and adding each element to x_line
		...
		int i = 0;
		while (i < width && (candidatesforx + i)->hue != -1 && (i < numb_new_cand_found))
		{																			 // checks that we are not counting the end_candidate as an actual candidate
	...
	{ // iterating by row
		...
		int i = 0;
		while (i < width && (candidatesfory + i)->hue != -1 && (i < numb_new_cand_found))
		{																			 // checks that we are not counting the end_candidate as an actual candidate
